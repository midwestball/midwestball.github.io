...
# Use the appDataDir to create/update KI
app_data_dir = Path('/Users/ehfurgeson/.gemini/antigravity/knowledge/inference_v2_qmd')
app_data_dir.mkdir(parents=True, exist_ok=True)
artifacts_dir = app_data_dir / 'artifacts'
artifacts_dir.mkdir(exist_ok=True)

# Write the actual KI markdown
ki_md_path = artifacts_dir / 'inference_v2_debugging.md'
with open(ki_md_path, 'w') as f:
    f.write("""# ballnet inference_v2.qmd Debugging and Pipeline Knowledge

Knowledge regarding module imports, common gotchas, and specific bugs uncovered in the `ballnet` execution pipeline (`inference_v2.qmd` / data pipeline). Please review this before editing scripts dealing with the `GATv2TCN` model or the `X_seq` data.
...
metadata_path = app_data_dir / "metadata.json"
now = datetime.now(timezone.utc).isoformat()
metadata = {
    "title": "Debugging and Fixing inference_v2.qmd and Data Pipeline",
    "summary": "Key gotchas and historical knowledge for working with the ballnet GATv2TCN pipeline. Includes the X_seq double-normalization bug, importing custom local model modules, tracking log KeyErrors, and QMD syntax errors.",
    "created_at": now,
    "last_updated": now,
    "references": []
}
...
print("KI inference_v2_qmd created successfully.")
