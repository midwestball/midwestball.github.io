...
Ballnet publishes one precomputed board per publishable group at `leaderboards/{season}/w{week}/{group}.json`, built from Stage E `ytd_*_pct.parquet`. Rows still include `denomYtd` for ramp–hold parity and fallback. the frontend loads that artifact from Storage and sorts client-side; percentiles are already oriented so Best = high percentile. Min-volume slider defaults to ramp–hold `minNBase × min(completedWeek, 5)` (`board.completedWeek`, else seasons/current, else `asOfWeek`) and filters on catalog `volumeStatId` joined from the sibling counting-stat rows on the same board (`denomYtd` only when `volumeStatId` is unset or the sibling join misses).
...
- Recompute slider floor in the frontend from catalog + week — do not trust a separate published `minN` field.
...
- Search **Fantasy Rank** is not a board `stats` key — sort `fantasyPosRank` already on each row.
- Search WR / TE filters load the `pass_catcher` board and keep `position === "WR"` / `"TE"`; do not publish split WR/TE leaderboards.