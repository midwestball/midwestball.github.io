### Context
The frontend’s prototype chrome used rounded cards, pills, and drop shadows, which read as a generic dashboard and wasted vertical space on dense percentile stacks.
...
- `--radius` in `frontend/src/app/globals.css` stays `0`. Do not reintroduce `--radius-lg` as visual policy.
