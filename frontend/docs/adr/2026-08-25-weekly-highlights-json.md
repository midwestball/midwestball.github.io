...
### Decision
Ballnet Stage H reads the Stage B spine, scores curated catalog stats with oriented z-scores vs **season-of-games** peers (`ballnet.scoring`), and publishes one board per week at `highlights/{season}/w{week}.json` plus allowlist KDEs under `dists/league_weekly/`. The frontend loads those objects via public Storage `fetch` and renders them on `/`. Payload: `schemaVersion: 1`, `top[]`, `byGroup`, rows with `playerId`, `statId`, `value`, `zScore`, `peerN`, `oneInN`, `rank`.
...
- **Unchanged:** No Stage G coupling; no The frontend SQL client.
