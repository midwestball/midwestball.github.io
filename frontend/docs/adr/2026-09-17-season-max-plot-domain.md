...
### Decision
Ballnet `_expand_domain` sets published league (and weekly highlight) domains to **`[sample_min, sample_max]`** for that `(season, as_of_week, group, stat)`. Empty sample keeps the catalog domain. Axes stay raw-value ordered (low→high): for `higherIsBetter`, left = worst / 0th and right = best / 100th; for lower-is-better, worst sits at the high/right end without flipping the chart. Knowball continues to trust published `xMin`/`xMax`.
...
- **Unchanged:** Reflection walls still use catalog `lower_bound` / `upper_bound`; Knowball does not invent domains or flip axes by `higherIsBetter`.
