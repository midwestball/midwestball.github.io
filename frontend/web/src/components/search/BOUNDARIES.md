...
- When the loaded board has any `denomYtd`, filter with `denomYtd >= min`. When every row lacks `denomYtd`, fall back to `qualified` (Ballnet ramp–hold) and keep the slider disabled.
...
- Stats with no spine source yet (`red_zone_*`, `route_pct`, …) publish null `denomYtd` and `qualified: false` → empty ranked list + disabled slider (“No volume data for this stat yet”).
- Slider is interactive only when `max(denomYtd) >` ramp–hold floor; otherwise it stays grayed at the floor.
