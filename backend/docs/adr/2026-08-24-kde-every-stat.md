### Context
Discrete catalog stats were published as fixed-bin histograms; the frontend needed one league shape so inclusive CDF and chart y-axis match.
...
### Consequences
- Stage E percentiles always use `_kde_cdf` (frontend `kdeCdf`). Discrete percentiles change; republish pages with league files.
