...
- Storage uploads (`upload-storage`) go to public bucket `knowball-public` under `index/`, `pages/`, and `league/`. Free plan is ~1 GB — skip `--also-current` (duplicates the season). Dashboard drag-drops often land at bucket root; re-upload with `--index` to get `index/*.json`.
